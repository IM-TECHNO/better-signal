...
public class ThumbnailView extends FrameLayout {
  ...
  public ThumbnailView(final Context context, AttributeSet attrs, int defStyle) {
    ...
    this.shimmer                 = findViewById(R.id.thumbnail_shimmer);
  ...
  /**
   * Starts a looping diagonal shimmer sweep over the thumbnail while its attachment is still
   * downloading, instead of leaving a blank placeholder for the whole transfer.
   */
  private void startShimmer() {
    ...
    shimmerAnimator = ValueAnimator.ofFloat(-1f, 1f);
    shimmerAnimator.setDuration(1100);
    shimmerAnimator.setStartDelay(200);
    shimmerAnimator.setRepeatCount(ValueAnimator.INFINITE);
    shimmerAnimator.setInterpolator(new LinearInterpolator());
    shimmerAnimator.addUpdateListener(animation -> {
      float fraction = (float) animation.getAnimatedValue();
      shimmer.setTranslationX(fraction * shimmer.getWidth());
    });
